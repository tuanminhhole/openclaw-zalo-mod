...
-----END PUBLIC KEY-----`;
        ...
        async function verifyLicenseKey(key) {
            ...
            // 1. Local Dev Keys for Testing (valid for 30 days from activation date)
            ...
            if (key.startsWith('DEV-OP-') && key.length > 12) {
                return { valid: true, plan: 'personal', expiry: getTrialExpiry(), deviceId };
            }
            if (key.startsWith('DEV-TEAM-') && key.length > 12) {
                return { valid: true, plan: 'team', expiry: getTrialExpiry(), deviceId };
            }
            if (key.startsWith('DEV-LIFETIME-') && key.length > 12) {
                return { valid: true, plan: 'lifetime', expiry: getTrialExpiry(), deviceId };
            }
