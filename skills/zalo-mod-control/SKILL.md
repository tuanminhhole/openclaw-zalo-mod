...
description: Điều khiển plugin Zalo Mod bằng ngôn ngữ tự nhiên — bật/tắt mute, silent, welcome, follow theo nhóm; đọc và tổng hợp lịch sử chat nhóm; xem/duyệt/từ chối việc tồn đọng trên kanban; chạy các action của dashboard. Dùng khi owner nhờ đổi cấu hình bot Zalo, hỏi trạng thái nhóm, hỏi còn việc gì chưa xong, hoặc tổng hợp nội dung chat.
---
...
Sáu tool này **chỉ owner của bot dùng được** — host tự chặn, bạn không cần tự kiểm tra quyền. Nếu tool trả lỗi "chỉ owner", hãy nói thật với người dùng là họ không có quyền.
...
| `zalo_mod_settings` | Bật/tắt `muted`, `silent`, `welcome`, `follow`, `tracking`, `pendingAuto`, `autoSummary`, `backlogInclude` cho một hoặc nhiều nhóm. Tương đương bấm badge trên dashboard. |
...
| `zalo_mod_tasks` | Xem/duyệt/từ chối/đổi cột **việc tồn đọng** (kanban "Công việc"). Dùng khi owner hỏi "nhóm X còn việc gì", hoặc nói một việc đã xong/đã duyệt/không đúng. |
...
### Việc tồn đọng — dùng `zalo_mod_tasks`

Đây là kanban "Công việc" trên dashboard: `pending_review` (🤖 AI đề xuất, chờ người duyệt) →
`todo`/`doing`/`blocked`/`done`. Owner nói việc bằng TÊN, không bằng id — bạn phải tự khớp.

```
zalo_mod_tasks { operation: "list", groups: ["ME ME"] }
  → { tasks: [{ id, title, status, groupName, assignee, dueAt, source }] }

// việc đã xong
zalo_mod_tasks { operation: "status", title: "gọi điện cho Trường Tuệ", status: "done" }

// duyệt việc AI đề xuất (giữ nguyên cột, rơi vào "Cần làm")
zalo_mod_tasks { operation: "approve", title: "báo giá lô hàng mới" }

// duyệt VÀ xong luôn trong một lượt
zalo_mod_tasks { operation: "approve", title: "báo giá lô hàng mới", status: "done" }

// từ chối việc AI đề xuất sai/nhiễu
zalo_mod_tasks { operation: "reject", title: "ngủ sớm hơn" }
```

**⛔ `title` khớp GẦN ĐÚNG, không cần chính xác từng chữ.** Ra nhiều hơn một kết quả → tool trả
`suggestions`, KHÔNG được tự chọn đại một cái — hỏi lại owner rõ hơn (thêm tên nhóm vào `groups`
để thu hẹp), rồi gọi lại. Ra 0 kết quả → nói thật là không tìm thấy, đừng bịa là "đã xong".

`operation: "status"` chỉ nhận `todo`/`doing`/`blocked`/`done` — muốn đưa việc từ "Chờ xác nhận"
sang cột khác thì dùng `operation: "approve"` kèm `status`, không dùng `"status"` suông.

"Từ chối" **không xoá việc** — chỉ ẩn khỏi kanban và báo cáo. Nếu quá 30 ngày mà AI vẫn thấy owner
nhắc lại đúng việc đó, nó sẽ tự quay về "Chờ xác nhận" (không tự duyệt) — không phải bug, đừng báo owner là lỗi.
...
### "Nhóm Kinh Doanh còn việc gì chưa xong?" / "Việc gọi điện cho Trường Tuệ xong rồi"

```
zalo_mod_tasks { operation: "list", groups: ["Kinh Doanh"] }
zalo_mod_tasks { operation: "status", title: "gọi điện cho Trường Tuệ", status: "done" }
```

Nếu kết quả có `suggestions` (khớp `title` ra nhiều hơn một việc) → đọc tên + nhóm của từng cái, hỏi lại owner chọn đúng cái nào. **Đừng tự chọn.**
